...
exports.checkMLBridge = checkMLBridge;
exports.mlEnhance = mlEnhance;
...
async function mlEnhance(product) {
    if (!ML_ENABLED)
        return null;
    try {
...
function validateCSVRow(raw, rowIndex) {
    ...
    if (errors.length > 0)
        return { valid: false, errors, warnings: [], row: null };
...
// ── Core box selector ──────────────────────────────────────────────
function selectOptimalBox(product, catalog, mlResult) {
    ...
    const allCosts = smallerFittingBoxes.map(b => {
        const dw = (0, types_1.calcDimWeight)(b.length_cm, b.width_cm, b.height_cm);
        return b.cost_per_box_usd + (0, types_1.calcShippingCost)(dw, product.weight_kg * product.quantity, product.shipping_zone);
    });
    ...
    const TOLERANCE = 0.1;
    const isSameBox = Math.abs(best.box.length_cm - product.used_box_length) < TOLERANCE &&
...
async function bulkOptimize(rawRows, catalog, onProgress) {
    ...
    for (let i = 0; i < rawRows.length; i += CHUNK) {
        ...
        const validRows = chunk.map((r, idx) => validateCSVRow(r, i + idx)).filter(v => v.valid && v.row).map(v => v.row);
        // Bulk ML call
        let mlResults = {};
