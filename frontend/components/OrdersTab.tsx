...
export default function OrdersTab({ runId, companyId }: { runId: string, companyId: string }) {
  ...
  useEffect(() => {
        ...
        if (isMounted) {
          ...
          if (!summary) {
            ...
            if (runData) {
              setSummary({
                runId,
                totalOrders: runData.total_products || 0,
                optimizedCount: rows.filter(r => r.fit_status === 'optimized').length,
    ...
    let channel: any
    if (supabase.channel) {
      channel = supabase.channel(`public:optimized_orders:run_id=eq.${runId}`)
        .on(
          'postgres_changes',
          { event: 'INSERT', schema: 'public', table: 'optimized_orders', filter: `run_id=eq.${runId}` },
          (payload: any) => {
            if (isMounted) {
              setRows(prev => {
